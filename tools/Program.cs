// This is a tool I threw together to test my API for a full month worth of data.
// Without the premium API I needed a way to verify/test my program, and this generator allows
// me to generate test cases. It's not perfect, but without direct access to alphavantage it serves its'
// purpose. It especially helped me when I ran out of API calls.
using System;
...
{
    // using UTC times for simplicity, but I don't believe it matters. They broadly match market hours.
    var open = d.Date.AddHours(13).AddMinutes(30); 
    var close = d.Date.AddHours(20);               

    // It was either more complex calculations or something more static. I opted for more general pattern
    ...
    for (var t = open; t <= close; t = t.AddMinutes(intervalMinutes)){
        var delta = (decimal)(rnd.NextDouble() - 0.5) * 0.8m;
        var o = price;
        var h = Math.Max(o, o + Math.Abs(delta) + 0.2m);
        var l = Math.Min(o, o - Math.Abs(delta) - 0.2m);
        var c = o + delta;
        var v = rnd.Next(20_000, 200_000);
